...
exceedsDaysInMonth()
{
	...
	if [ $2 -lt 1 -o $2 -gt $days ] ; then
		...
		return 0 # the day numbe is valid.
	fi
}
...
isLeapYear()
{
	...
	#  1 otherwise.
	# The formula for cehcking whether a year is a leap year is:
	#  1.  Years not divisible by 4 are not leap years
	#  2.  Years divisible by 4 and by 400 are leap years
	#  3.  Years divisible by 4, anot divisible by 400, but divisible
...
if [ $# -ne 3 ] ; then
	echo "Usuage: $0 month day year" >&2
	echo "Typical input formats are August 3 1962 and 8 3 1962" >&2
	exit 1
fi

# Normalize date and store the return value to cehck for errors
...
# split the normalized date formate, where
...
# now that we have a normalized date, let's chekc wheter the 
#  the day value is legal and valid (e.g., not Jan 36)

if ! exceedsDaysInMonth $month "$2" ; then
	if [ "$month" = "Feb" -a "$2" -eq "29" ] ; then
		if ! isLeapYear $3 ; then
			echo "$0: $3 is not a leap year, Feb doesn't have 29 days." >&2
			exit 1
		fi
	else echo "$0: bad day value: $month doesn't have $2 days." >&2
		exit 1
	fi
fi
